...
from lib.training import perform_inductive_margin_training
from lib.eval import do_production_eval
from ogb.linkproppred import PygLinkPropPredDataset
from lib.transforms import VALID_TRANSFORMS

from lib.utils import add_node_feats, do_node_inductive_edge_split, is_small_dset, merge_multirun_results, set_random_seeds
...
flags.DEFINE_bool('batch_links', False, 'Whether or not to batch links (not implemented for training yet)')
...
def main(_):
    ...
    wandb.init(project="ml-gcn-inductive", config={'model_name': get_full_model_name(), **FLAGS.flag_values_dict()})
    ...
    if isinstance(dataset, PygLinkPropPredDataset):
        ...
        training_data, val_data, inference_data, data, test_edge_bundle, negative_samples = do_node_inductive_edge_split(
            dataset=dataset, split_seed=FLAGS.split_seed, small_dataset=is_small_dset(FLAGS.dataset))  # type: ignore
    ...
    for run_num in range(FLAGS.num_runs):
        ...
        encoder, representations, time_bundle = perform_inductive_margin_training(training_data, val_data,
                                                                                  inference_data, data,
                                                                                  test_edge_bundle, negative_samples,
                                                                                  OUTPUT_DIR, writer, device,
