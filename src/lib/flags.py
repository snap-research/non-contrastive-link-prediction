...
def define_shared_flags(model_group):
    ...
    flags.DEFINE_string(
        'model_name_prefix', '', 'Prefix to prepend to the output directory'
    )
    flags.DEFINE_bool(
        'debug',
        False,
    ...
    flags.DEFINE_float(
        'weight_decay', 1e-5, 'The value of the weight decay for training.'
    )
    flags.DEFINE_integer(
        'epochs', get_default('epochs'), 'The number of training epochs.'
    )
    ...
    flags.DEFINE_integer(
        'link_nn_epochs', 10000, 'Number of epochs in the NN for evaluation.'
    )
    flags.DEFINE_bool(
        'batch_links', False, 'Whether or not to perform batching on links.'
    )
    flags.DEFINE_integer('link_batch_size', 64 * 1024, 'Batch size for links.')
    flags.DEFINE_enum(
        'trivial_neg_sampling',
        'false',
        ['true', 'false'],
...
def define_aug_flags():
    ...
    flags.DEFINE_float('drop_edge_p_1', 0.0, 'Probability of edge dropout 1.')
    flags.DEFINE_float('drop_feat_p_1', 0.0, 'Probability of node feature dropout 1.')
    flags.DEFINE_float('drop_edge_p_2', 0.0, 'Probability of edge dropout 2.')
    flags.DEFINE_float('drop_feat_p_2', 0.0, 'Probability of node feature dropout 2.')
    flags.DEFINE_enum(
        'graph_transforms',
        'standard',
    ...
    flags.register_validator(
        'drop_edge_p_1', between_zero_one, 'must be between 0 and 1'
    )
    flags.register_validator(
        'drop_feat_p_1', between_zero_one, 'must be between 0 and 1'
    )
    flags.register_validator(
        'drop_edge_p_2', between_zero_one, 'must be between 0 and 1'
    )
    flags.register_validator(
        'drop_feat_p_2', between_zero_one, 'must be between 0 and 1'
    )
