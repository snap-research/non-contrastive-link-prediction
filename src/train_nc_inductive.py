...
from lib.eval import do_production_eval, perform_nn_link_eval
...
def main(_):
    ...
    for run_num in range(FLAGS.num_runs):
        ...
        if FLAGS.base_model == 'bgrl':
            encoder, representations, time_bundle = perform_bgrl_training(dataset=dataset,
                                                                          data=training_data,
                                                                          output_dir=OUTPUT_DIR,
                                                                          representation_size=representation_size,
                                                                          device=device,
                                                                          num_eval_splits=num_eval_splits,
            ...
            time_bundle = None
            encoder, representations, time_bundle = perform_cca_ssg_training(data=training_data.to(device),
                                                                             output_dir=OUTPUT_DIR,
                                                                             device=device,
                                                                             input_size=input_size,
                                                                             has_features=has_features,
            ...
            encoder, representations, time_bundle = perform_gbt_training(training_data.to(device), OUTPUT_DIR, device,
                                                                         input_size, has_features, g_zoo)
            # del encoder
            log.info('Finished training')
        elif FLAGS.base_model == 'simsiam':
            time_bundle = None
            encoder, representations = perform_simsiam_training(data=training_data.to(device),
                                                                output_dir=OUTPUT_DIR,
                                                                representation_size=representation_size,
                                                                device=device,
                                                                input_size=input_size,
